...
func GetDSN(tb testing.TB) *mysql.Config {
	envdsn := os.Getenv("TEST_DSN")
	cfg, err := mysql.ParseDSN(envdsn)
	if err != nil {
		fmt.Printf("%s cannot parse the DSN %q: %s", caller(), envdsn, err)
		tb.FailNow()
	}
	return cfg
}
...
func MySQLGreatherThanOrEqual(tb testing.TB, v string) bool {
	cmpVersion, err := version.NewVersion(v)
	if err != nil {
		fmt.Printf("%s invalid input version %q: %s\n\n", caller(), v, err)
		tb.FailNow()
	}
	mysqlVersion := GetVersion(tb, GetMySQLConnection(tb))

	if !mysqlVersion.LessThan(cmpVersion) {
		return true
	}
	return false
}

func SkipIfNotEqual(tb testing.TB, v string) {
	cmpVersion, err := version.NewVersion(v)
	if err != nil {
		fmt.Printf("%s invalid input version %q: %s\n\n", caller(), v, err)
		tb.FailNow()
	}
	mysqlVersion := GetMinorVersion(tb, GetMySQLConnection(tb))

	if mysqlVersion.LessThan(cmpVersion) {
		tb.Skipf("This test needs MySQL version %s+. Skipping", v)
	}
}

func SkipIfLessThan(tb testing.TB, v string) {
	cmpVersion, err := version.NewVersion(v)
	if err != nil {
		fmt.Printf("%s invalid input version %q: %s\n\n", caller(), v, err)
		tb.FailNow()
...
func SkipIfGreatherThan(tb testing.TB, v string) {
	cmpVersion, err := version.NewVersion(v)
	if err != nil {
		fmt.Printf("%s invalid input version %q: %s\n\n", caller(), v, err)
		tb.FailNow()
	...
	if mysqlVersion.GreaterThan(cmpVersion) {
		tb.Skipf("This test needs MySQL version < %s. Skipping", v)
	}
}
